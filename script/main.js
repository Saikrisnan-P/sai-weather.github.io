import { loadData, httpCall } from "/script/httpsfunctions.js"; //Promise for http
...
(async function() {             
  ...
  class CityCards extends City {
    ...
    getCardsHTML() {
      return `<div class="tCity"> 
                        <div class="flex1Items">                  
                            ...
                            <div class="cithum"><img src="dependables/Weather Icons/humidityIcon.svg" alt="weather"><span class="hpValue">${
                              this.humidity
                            }</span></div>
                            <div class="citwind"><img src="dependables/Weather Icons/precipitationIcon.svg" alt="weather"><span class="hpValue">${
                              this.precipitation
                            }</span></div>
                        </div>
                        <div class="flex2Items">
                            <img id="citweat" src="dependables/Weather Icons/${weatherIcon}.svg" alt="weather">
    ...
    getGridHTML() {
      return `<div class="gBox">
            ...
            <div class="gridHumi"><img src="dependables/Weather Icons/humidityIcon.svg" alt="weather">${
              this.humidity
            }</div>
          </div>
          `;
    ...
    getBG() {
      return `url('dependables/Icons for cities/' + ${this.cityName.toLowerCase()} + '.svg')`;
    }
  }
  
  let timeZone; //These three are global vars as they are used in selectedCity
  ...
  function selectedCityFunction(cityName) {
    ...
    if (cityName === "") {
      cityIcon.src ="dependables/General Images & Icons/warning.svg";
    } else {
      cityIcon.src ="dependables/Icons for cities/" + cityName.toLowerCase() + ".svg";
  ...
  var updateForecast = function(meridien, currHour){
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (nextTemp[i].innerHTML <= 18) {
        nextImg[i].src = "dependables/Weather Icons/rainyIcon.svg";
      } else if (nextTemp[i].innerHTML > 18 && nextTemp[i].innerHTML <= 22) {
        nextImg[i].src = "dependables/Weather Icons/cloudyIcon.svg";
      } else if (nextTemp[i].innerHTML < 22 && nextTemp[i].innerHTML <= 29) {
        nextImg[i].src = "dependables/Weather Icons/windyIcon.svg";
      } else {
        nextImg[i].src = "dependables/Weather Icons/sunnyIcon.svg";
      }
      if (i !== 0) {
  ...
  var setDateTime = function(){  
    ...
    if(meridien=="AM"){
      amPm.src="dependables/General Images & Icons/amState.svg";
    }
    else {
      amPm.src="dependables/General Images & Icons/pmState.svg";
    }
  }
  ...
  function addCards() {
    ...
    for (let h = 0; h < minVal; h++) {
      currentCity[h].style.backgroundImage =
        "url('dependables/Icons for cities/" +
  ...
  contToggle.addEventListener("click", function () {
    ...
    if (contArrow.src.split('/').slice(-1) =="arrowDown.svg") {
      ...
      contArrow.src =
        "dependables/General Images & Icons/arrowUp.svg";
    } else {
      continentCities.sort(function (a, b) {
        if (a.timeZone.split("/")[0] > b.timeZone.split("/")[0]) return 1;
        else return -1;
      ...
      contArrow.src =
        "dependables/General Images & Icons/arrowDown.svg";
    }
    addGridCitites();
  });
